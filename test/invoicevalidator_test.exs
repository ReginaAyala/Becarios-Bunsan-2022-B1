defmodule InvoiceValidatorTest do
  ...
  data = [
    {"72 hrs before", "Mexico/BajaNorte", ~N[2022-03-20 13:06:31],
     "{:error, Invoice was issued more than 72 hrs before received by the PAC}"},
    {"72 hrs before", "Mexico/BajaSur", ~N[2022-03-20 14:06:31],
     "{:error, Invoice was issued more than 72 hrs before received by the PAC}"},
    ...
    {"5 mns after", "Mexico/BajaNorte", ~N[2022-03-23 14:11:36],
     "{:error, Invoice is more than 5 mins ahead in time}"},
    {"5 mns after", "Mexico/BajaSur", ~N[2022-03-23 14:11:36],
     "{:error, Invoice is more than 5 mins ahead in time}"},
    {"5 mns after", "Mexico/General", ~N[2022-03-23 15:11:36],
     "{:error, Invoice is more than 5 mins ahead in time}"},
    {"5 mns after", "America/Cancun", ~N[2022-03-23 16:11:36],
     "{:error, Invoice is more than 5 mins ahead in time}"}
  ]
      ...
      if @a == "5 mns after" do
        if @d == "{:error, Invoice is more than 5 mins ahead in time}" do
          assert {:error, "Invoice is more than 5 mins ahead in time"} ==
                   InvoiceValidator.validate_dates(datetime(@c, @b), @pacdate)
        else
